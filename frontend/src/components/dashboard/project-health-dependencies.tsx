...
type Row = DependencyRow & { id: string };

export function ProjectHealthDependencies() {
  ...
  const {
    ...
    error,
    refetch,
  } = useProjectHealthDependencies({ ...filters, search: search || undefined, skip, limit: PAGE_SIZE });
  ...
  const columns: RegisterColumn<Row>[] = [
    ...
    {
      key: "dependency_title",
      label: "Dependency",
      render: (row) => <span className="font-semibold text-slate-900">{row.dependency_title}</span>,
    },
    { key: "depends_on", label: "Dependency On" },
    { key: "owner_name", label: "Owner" },
    { key: "due_date", label: "Due Date", render: (row) => formatDate(row.due_date) },
    { key: "status", label: "Status", badge: true },
  ];

  const rows: Row[] = (data?.items ?? []).map((row) => ({ ...row, id: row.dependency_id }));

  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-6">
      <header className="flex flex-col gap-2">
        <BackToProjectHealth />
        <h1 className="text-3xl font-bold tracking-tight text-slate-900">Dependencies</h1>
        <p className="text-slate-500">Cross-team and cross-project dependency tracking.</p>
      </header>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        <StatTile label="Open Dependencies" value={summary?.dependencies.open_count ?? "—"} />
        <StatTile label="Critical" value={summary?.dependencies.critical_count ?? "—"} accentClassName="border-t-red-500" />
        <StatTile label="Overdue" value={summary?.dependencies.overdue_count ?? "—"} accentClassName="border-t-amber-500" />
      </div>

      {isError ? (
        <ErrorBlock title="Couldn't load dependencies." error={error} onRetry={() => refetch()} />
      ) : (
        <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <ProjectHealthExportButton
              filename="project-health-dependencies"
              columns={columns}
              fetchAll={() =>
                fetchAllProjectHealthRows<Row>(PROJECT_HEALTH_LIST_PATHS.dependencies, {
                  ...filters,
          ...
          <RegisterTable items={rows} columns={columns} emptyLabel={isLoading ? "Loading…" : "No dependencies found."} />
