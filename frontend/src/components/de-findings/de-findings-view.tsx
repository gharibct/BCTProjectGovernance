...
import { DeFindingsFilterBar } from "./de-findings-filter-bar";
import { DeFindingsDrawer, type DrawerState } from "./de-findings-drawer";
...
export function DeFindingsView() {
  const [filters, setFilters] = React.useState<DeFindingsFilter>({ status: "Active" });
  ...
  const list = useDeFindings({ ...filters, skip, limit: PAGE_SIZE });
    ...
    <div className="mx-auto flex max-w-7xl flex-col gap-6">
      <header className="flex flex-wrap items-end justify-between gap-3 border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Monitor and manage project findings</p>
        </div>
        <Button
          onClick={() => setDrawer({ mode: "create" })}
          disabled={!canWrite}
          className="h-11 gap-2 bg-[#0f172a] px-5 text-sm font-semibold text-white hover:bg-[#1e293b]"
        >
      ...
      {list.isError ? (
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <RegisterTable
            ...
            onRowClick={(r) => setDrawer({ mode: "detail", findingId: r.id })}
          />
          <PaginationBar
            skip={skip}
            limit={PAGE_SIZE}
