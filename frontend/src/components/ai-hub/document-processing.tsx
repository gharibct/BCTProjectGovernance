...
// Weekly period when no ?period= is in the URL yet (e.g. a direct link).
// Split out from DocumentProcessing because useSearchParams requires a
...
function DocumentProcessingInner({ projectId }: { projectId: string }) {
  ...
  const addFiles = (files: FileList | null) => {
    ...
    if (!resolvedPeriodId) {
      showError("No reporting period available yet — try again in a moment.");
      return;
    }
    ...
    Array.from(files).forEach((file) => {
      uploadDocument.mutate(
        { file, context: "reporting", periodId: resolvedPeriodId },
...
export function DocumentProcessing() {
  ...
  return (
    <Suspense fallback={null}>
      <DocumentProcessingInner projectId={projectId} />
    </Suspense>
  );
}
