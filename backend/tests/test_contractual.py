...
async def test_list_commitments_returns_200_for_de_regardless_of_ownership(client, override_auth):
...
async def test_list_commitments_rejects_team_member_with_no_ownership(client, override_auth):
...
async def test_list_milestones_returns_200_for_de_regardless_of_ownership(client, override_auth):
    headers = override_auth(RoleCode.DELIVERY_EXCELLENCE, get_map=_PROJECT_GET_MAP)
...
async def test_list_milestones_rejects_team_member_with_no_ownership(client, override_auth):
    headers = override_auth(RoleCode.TEAM_MEMBER, get_map=_PROJECT_GET_MAP)
    response = await client.get(f"/api/v1/projects/{_PROJECT_ID}/milestone-payments", headers=headers)
    assert response.status_code == 403
